...
- **Charts**: Recharts
- **Database**: SQLite via Prisma (swap `DATABASE_URL` for Postgres to go to production)

## Getting started

```bash
npm install
cp .env.example .env      # already done if you're reading this in a fresh clone from this session
...
| `npm run lint` | ESLint |
| `npm run db:push` | Push the Prisma schema to SQLite (no migration history) |
...
src/
  ...
  lib/
    ...
    filters.ts, db.ts, ingest.ts, serialize.ts, stats.ts, format.ts
  types/listing.ts          # shared TS types
prisma/
  schema.prisma             # Listing, ListingSnapshot, PriceChange, ScoringConfig, FilterPreset
