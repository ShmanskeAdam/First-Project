...
import { buildCompIndex, scoreListing, type ScoringInput } from "./dealScore";
 ...
 */
export async function scoreListingsById(targetIds: string[]): Promise<Map<string, DealScoreResult>> {
...
export async function scoreAllActiveListings(): Promise<{ ids: string[]; scores: Map<string, DealScoreResult> }> {
  const [config, universe] = await Promise.all([
    getScoringConfig(),
    prisma.listing.findMany({ select: UNIVERSE_SELECT }),
  ]);
  ...
  const scores = new Map<string, DealScoreResult>();
  for (const listing of scoringUniverse) {
    if (listing.status !== "ACTIVE") continue;
    activeIds.push(listing.id);
    scores.set(listing.id, scoreListing(listing, index, config));
  }
